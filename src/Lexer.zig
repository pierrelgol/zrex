...
test "C keyword regex" {
    // A simple alternation listing many C keywords.
    var lexer = Lexer.init("^(auto|break|case|char|const|continue|default|do|double|else|enum|extern|float|for|goto|if|int|long|register|return|short|signed|sizeof|static|struct|switch|typedef|union|unsigned|void|volatile|while)$");
    const tokens = try lexer.lex();
    for (tokens) |token| {
        std.debug.print("{}", .{token});
    }
